(function ($) {
    ...
    *********************************/
    ...
    $(".close__btn").on("click", function (event) {
        event.preventDefault();
        $(".flyoutCart").toggleClass("active");
        $(".flyoutCart").removeClass("highlight");
    });
...
$('#form-login').on('submit', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: "http://remedic.oakmis.com/customer-login",
        processData: false,
        contentType: false,
        data: formData,
        dataType: 'json',
        success: function (data) {
            if (data.success === 'SUCCESS') {
                 window.location = "http://remedic.oakmis.com";
            } else {
                //False Case: With error msg
                $('#wrongcredentials').empty();
                $('#wrongcredentials').show();
                $('#wrongcredentials').append('<div class="alert alert-danger"><p>Check Your Credentials and Try Again.</p></div>');  //$msg is the id of empty msg
            }
        },
        error: function (data) {
            $('#wrongcredentials').empty();
...
$('#signup-form').on('submit', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: "http://remedic.oakmis.com/signup",
        processData: false,
        contentType: false,
        data: formData,
        dataType: 'json',
        success: function (data) {
            console.log(data);
            if (data.success === 'SUCCESS') {
                $('#successmsg').append('<div class="alert alert-success"><p>You Have been Registered Successfully !!.</p></div>');
                window.setTimeout(function(){
                   // Move to a new location or you can do something else
                    window.location.href = "http://remedic.oakmis.com";
                }, 5000);
            } 
        },
        error: function (data) {
