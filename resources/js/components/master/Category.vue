            ...
            <div class="row">
                <div class="col-12 mb-2">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <h1 class="h2 mb-md-0 text-white fw-light">Categories</h1>
                        <div class="page-action">
                            <button class="btn d-none d-sm-inline-flex rounded-pill" type="button" @click="openCreatePannel()">
                                <span class="me-1 d-none d-lg-inline-block">Add Category</span>
                ...
                <div class="col-12" v-if="createPannel" style="margin-bottom: 10px;">
                    <div class="card">
                        <div class="card-header bg-transparent">
                            ...
                            <small class="text-muted">Fill the details about the category.</small>
                            </div> 
                        </div>
                        <form class="card-body">
                            <div class="row mb-3">
                                ...
                                <input type="text" class="form-control form-control-lg" v-model="categoryParam.name">
                                </div>
                            </div>
                            <div class="row mb-3">
                                <label class="col-md-3 col-sm-4 col-form-label">Description <sup class="text-danger">*</sup></label>
                                <div class="col-md-9 col-sm-8">
                                <textarea class="form-control" placeholder="Type description here.." rows="3" v-model="categoryParam.description"></textarea>
                                </div>
                            </div>
                            <div class="row mb-3">
                                <label class="col-md-3 col-sm-4 col-form-label">Slug <sup class="text-danger">*</sup></label>
                                <div class="col-md-9 col-sm-8">
                                    <input type="text" class="form-control form-control-lg" v-model="categoryParam.slug">
                                </div>
                            </div>
                            <div class="row mb-3">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-9 col-sm-8">
                                <select class="form-control form-control-lg" v-model="categoryParam.active">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header bg-transparent pb-0">
                            <div>
                            <h6 class="card-title mb-0">Manage Categories</h6>
                            <small class="text-muted">Manage details about the categories here.</small>
                            </div> 
                        </div>
                        <div class="card-body row-title mb-0">
                                <h5 class="fw-light mb-0">Categories
                        ...
                        <div class="card-body pt-0" v-if="categories.length > 0">
                            <table class="table align-middle card-table mb-0 categoryTable">
                                <thead>
                                    <tr>
                                        <th>Category ID</th>
                                        <th>Logo</th>
                                ...
                                <tbody>
                                    <tr v-for="category in categories" :key="category.id">
                                        ...
                                        <td>
                                            <img v-bind:src="'/storage/categories/'+category.thumbnail" width="50" height="50">
                                        </td>
                                        <td>{{ category.name }} 
                                            <span class="badge bg-success" v-if="category.active === 'Y'">Active</span>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-link btn-sm text-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit Role"  @click="showEditDialog(category)"><i class="fa fa-pencil"></i></button>
                                            <button type="button" class="btn btn-link btn-sm text-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete" @click="showDeleteAlert(category.id)"><i class="fa fa-trash"></i></button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div class="card-body col-12 text-center p-5" v-else> 
                            ...
                            <div class="mt-4 mb-3">
                                <span class="text-muted">No categories to show here use below to create one.</span>
                            </div>
                            <button type="button" class="btn btn-white border lift" @click="openCreatePannel()">Create Category</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    export default {
        ...
        updated(){
            if(this.isCreating){
                $('.categoryTable').addClass('nowrap').dataTable({
                    responsive: true,
                    paging: false,
                    searching: false,
                    ordering: true,
        ...
        methods : {
            ...
            searchUser(event){
                axios
                .get('/admin-api/categories?search='+event.target.value)
                .then(response => {
                    this.categories = response.data.data;
                    this.links = response.data.links;
                    this.total = response.data.total;
            ...
            showEditDialog(data){
                ...
                this.image = null
                this.isEditing = true
                this.categoryParam.id = data.id;
                this.categoryParam.name = data.name;
                this.categoryParam.slug = data.slug;
                this.categoryParam.description = data.description;
                this.categoryParam.active = data.active;
            ...
            updateCategory(){
                ...
                form.set('name',this.categoryParam.name);
                form.set('description',this.categoryParam.description);
                form.set('active',this.categoryParam.active);
                form.set('slug',this.categoryParam.slug);
                if(this.image != null)
                form.set('logo',this.image);
                axios
                    .post('/admin-api/categories/'+this.categoryParam.id+'?_method=PUT', form)
                    .then((res) => {
                        let index = this.categories.map(data => data.id).indexOf(this.categoryParam.id);
            ...
            createCategory() {
                ...
                axios
                    .post('/admin-api/categories', form)
                    .then(response => {
                        this.categoryParam = {}
