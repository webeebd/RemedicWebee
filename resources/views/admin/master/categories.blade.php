         ...
         <div class="col-12 mb-2">
            ...
            <div class="d-flex justify-content-between align-items-center">
               ...
               <div class="page-action">
			    @if(isset($category))
				 <a class="btn d-none d-sm-inline-flex rounded-pill" type="button" href="{{url('/admin/categories')}}">
...
<div class="page-body">
   <div class="container-fluid">
    <div class="row" id="catform" @if(isset($category) || $errors->any()) style="display:block;" @else style="display:none;" @endif>>
                <div class="col-12" style="margin-bottom: 10px;">
                    <div class="card">
                        ...
                        <div class="card-body">
                           @if(isset($category))
                           <form  action="{{route('categories.update',$category->id)}}" method="POST"  enctype="multipart/form-data" class="row">
                           @method('PUT')
                           @else
                           <form  action="{{url('admin/categories')}}" method="post" enctype="multipart/form-data" class="row">
                            @endif
                              @csrf
                              <input type="hidden" name="request_type" value="web">
                              <div class="col-sm-4">
                                 <label class="col-form-label">Name <sup class="text-danger">*</sup></label>
                                    <input type="text" class="form-control " name="name" required  @if(isset($category))  value="{{$category->name}}" @else value="{{ old('name') }}" @endif/>
                                    @if ($errors->has('name'))
                              ...
                              <div class="col-sm-4">
                                 <label class="col-form-label">Slug</label>
                                       <input type="text" class="form-control " name="slug" @if(isset($category))  value="{{$category->slug}}" @else value="{{ old('slug') }}" @endif/>
                                       @if ($errors->has('slug'))
                                       <label id="minmaxlength-error" class="error" for="minmaxlength">
                              ...
                              <div class="col-sm-4">
                                 ...
                                 <textarea class="form-control" placeholder="Type description here.." rows="3" name="description">  @if(isset($category)){{$category->description}}@endif </textarea>
                              ...
                              <div class="col-sm-4">
                                 ...
                                 <select class="form-select" aria-label="Default select example" name="active" required @if(isset($category))  value="{{$category->active}}" @else value="{{ old('active') }}" @endif>
                                    <option value="" selected>--- Select Status --</option>
                                    <option value="Y" @if(isset($category) && $category->active == 'Y') selected="selected" @endif>Active</option>
                                    <option value="N" @if(isset($category) && $category->active == 'N') selected="selected" @endif>In Active</option>
                                 </select>
                              </div>
                           <div class="text-end">
         ...
         <div class="col-12">
            <div class="card">
               ...
               <div class="card-body row-title mb-0">
                  <h5 class="fw-light mb-0">Categories <span class="fw-bold ms-2" id="catcount"></span></h5>
                  <div>
               ...
               <div class="card-body pt-0">
                  <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                     ...
                     <div class="row">
                        <div class="col-sm-12">
                           <table class="table align-middle card-table mb-0 categoryTable nowrap dataTable no-footer dtr-inline" id="myTable" style="width: 1302px;">
                              <thead>
                                 <tr>
                                   <th></th>
                                 <th>LOGO</th>
	...
	$.ajax({
        ...
        success : function(result){
			...
			$.each(result.data, function( index, value ) {
			   $('#databind').append('<tr>\n\
						<td>'+k+'</td>\n\
						<td><img src="'+ (value['thumbnail'] == null ? '/no_pic.png' : '/storage/categories/'+value['thumbnail'])+'" width="50" height="50"></td>\n\
